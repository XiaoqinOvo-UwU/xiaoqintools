...
{
    // Keep the "杂货铺" concept alive: settings live under AppData\小钦工具
...
void ConfigService::load()
{
    ...
    if (o.contains("gitee_repo")) m_giteeRepo = o.value("gitee_repo").toString();
    if (o.contains("gitee_token")) m_giteeToken = o.value("gitee_token").toString();
    f.close();
}
...
void ConfigService::save()
{
    ...
    o.insert("gitee_repo", m_giteeRepo);
    o.insert("gitee_token", m_giteeToken);
    QFile f(configPath());
