...
{
    return "C:/deepseek杂货铺/小钦工具/ai_memory.json";
}

void AiService::ensureMemory()
{
    QDir().mkpath("C:/deepseek杂货铺/小钦工具");
...
QString AiService::giteeRepo() { return ConfigService::instance().giteeRepo(); }
QString AiService::giteeToken() { return ConfigService::instance().giteeToken(); }
void AiService::setGiteeRepo(const QString &v) { ConfigService::instance().setGiteeRepo(v); }
void AiService::setGiteeToken(const QString &v) { ConfigService::instance().setGiteeToken(v); }

static QString copyAvatar(const QString &src, const QString &name)
{
    if (src.isEmpty() || !QFile::exists(src)) return QString();
    QDir().mkpath("C:/deepseek杂货铺/小钦工具");
    QString dest = "C:/deepseek杂货铺/小钦工具/" + name;
...
{
    QString p = "C:/deepseek杂货铺/小钦工具/user_avatar.png";
    return QFile::exists(p) ? p : QString();
}
QString AiService::aiAvatarPath()
{
    QString p = "C:/deepseek杂货铺/小钦工具/ai_avatar.png";
    return QFile::exists(p) ? p : QString();
}
